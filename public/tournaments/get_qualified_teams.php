...
try {
    ...
    $teams = $supabaseClient->select('round_teams', '*, teams(*, users(username), team_members(*, users(username)))', ['round_id' => $round_id, 'status' => 'qualified'], 'total_points.desc, kills.desc, placement.asc');
    
    $formatted_teams = [];
    if($teams) {
        foreach($teams as $team) {
            $team_data = $team['teams'];
            $team_data['kills'] = $team['kills'];
            $team_data['placement'] = $team['placement'];
            $team_data['total_points'] = $team['total_points'];
